...
#  
#  1.At first, I implemented the 'iaddl' and 'leave' instruction. 
#    The difference between the pipe-full file and the seq-full file is in this file, 
#    the 'leave' instruction has an effect on the control mechanisms. It has a risk to insert a bubble or a stall.
#    So the part of the register control should be modified.	 
#  2.Secondly, I completed the 'Load Forward' function.(It is the exercise 4.41 in CS:APP Page 363)
#    I add an additional bypass path to forward the memory output to the valA field in pipeline register M.
#    On the next clock cycle, this forwarded value can then be written to memory.This can deal with some special 'Load/Use Hazard'.
#    So I can use the 'rmmovl' instruction just behind the 'mrmovl' instruction which use the same register without the hazard of inserting a bubble.
#    To achieve this, we should add a new definition: M_srcA and modify the register control part in order to 
#     not insert a bubble in case of this special condition.
#
####################################################################
...
#mark
intsig M_srcA 'ex_mem_curr->srca'	
