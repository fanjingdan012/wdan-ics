/*
 * mtest.c - test harness for the CS:APP malloc lab
 *
...
/* Misc */
#define MAXLINE 128      /* max string size */
#define HDRLINES 4       /* number of header lines in a trace file */
#define LINENUM(i) (i+5) /* cnvt trace request nums to line nums (origin unity)*/
...
/*
 * List of Default tracefiles. 
 * Modify this if you want to add or delete traces.
 */
...
int main(int argc, char **argv)
{
     ...
     */
    while ((c = getopt(argc, argv, "f:hvVal")) != EOF) {
        switch (c) {
        case 'f': /* Use one specific trace file only */
            num_tracefiles = 1;
            if ((tracefiles = realloc(tracefiles, sizeof(char *))) == NULL)
...
static trace_t *read_trace(char *filename)
{
	...
	unix_error("fopen failed in read_trace");
